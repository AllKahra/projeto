...
# Flask app for serving frontend and API
app = Flask(__name__)
...
def print_main_menu():
    ...
    print("      Start web interface to view results")
    print()
...
def parse_scan_output(output_lines, scan_type):
    """Parse scan output lines and convert to structured JSON format."""
    results = []
    finding_id = 1
    
    for line in output_lines:
        # Example parsing logic (customize based on actual output format)
        if scan_type == "naabu":
            # Assuming naabu output like: "192.168.1.100:80 open"
            match = re.match(r'(\S+):(\d+)\s+open', line)
            if match:
                ...
                results.append({
                    ...
                    "severity": "medium",
                    "tool": "naabu",
                    "details": f"Port {port} is open on {ip}"
                })
                finding_id += 1
                
        elif scan_type == "httpx":
            # Assuming httpx output like: "http://example.com [200] [Apache]"
            match = re.match(r'(\S+)\s+\[(\d+)\]\s+\[(.*?)\]', line)
            if match:
                url, status, tech = match.groups()
                severity = "low" if int(status) == 200 else "medium"
                results.append({
                    "id": finding_id,
                    "title": f"HTTP Service - Status {status}",
                    "location": url,
                    "severity": severity,
                    "tool": "httpx",
                    "details": f"HTTP service running with status {status}, tech: {tech}"
                })
                finding_id += 1
                
        elif scan_type == "nuclei":
            # Assuming nuclei output like: "[critical] CVE-2021-1234 http://example.com"
            match = re.match(r'\[(\S+)\]\s+(\S+)\s+(\S+)', line)
            if match:
                severity, title, location = match.groups()
                results.append({
                    "id": finding_id,
                    "title": title,
                    "location": location,
                    "severity": severity,
                    "tool": "nuclei",
                    "details": f"Vulnerability {title} detected at {location}"
                })
                finding_id += 1
    
    return results

def save_json_output(output_lines, scan_type, result_dir):
    """Save scan results as JSON in the result directory."""
    json_data = parse_scan_output(output_lines, scan_type)
    json_file = os.path.join(result_dir, f"{scan_type}_results.json")
...
def run_scan(scan_type, target, **kwargs):
    ...
    print(f"[OUTPUT FORMAT]  TEXT + JSON")
    ...
    print(f"  Output format:    {'JSON + TEXT' if json_output else 'TEXT'}")
        ...
        if save_output:
            ...
            try:
                ...
                if result_dirs:
                    ...
                    # Save JSON output
                    save_json_output(output_lines, scan_type, latest_dir)
                    
                    # List key files
                    for filename in ['comprehensive_scan_report.txt', 'naabu_results.txt', 'httpx_results.txt', 'nuclei_results.txt', f"{scan_type}_results.json"]:
                        filepath = os.path.join(latest_dir, filename)
                        if os.path.exists(filepath):
                            file_size = os.path.getsize(filepath)
                            print(f"[FILE] {filename} ({file_size:,} bytes)")
                    
                    print(f"[ACCESS] Use menu option [4] to view detailed results or visit http://localhost:5000")
            except OSError as e:
...
def show_help():
    ...
    print("  • Web interface: Select option [9] or visit http://localhost:5000")
    print()
    print("EXAMPLES:")
